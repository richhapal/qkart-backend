...
const generateToken = (userId, expires, type, secret = config.jwt.secret) => {
  // const expire={expiresIn:expires};
...
const generateAuthTokens = async (user) => {
  // const generatedToken=generateToken(user["_id"],config.jwt.accessExpirationMinutes*60,tokenTypes.ACCESS,config.jwt.secret);
  // const accessTokenExpires=config.jwt.accessExpirationMinutes*60+Date.now()/1000; 
  
  // // const time=new Date(expireTime*1000);
